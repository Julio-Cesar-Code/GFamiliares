...
nextYear=pd.read_csv('./2023F.csv', sep=',')
...
for i in range(0,71):
    ...
    # table4.cell(0,2).text=str(round(pastYear.iloc[i,2]*0.75*8/10000*15+pastYear.iloc[i,2]*0.25*7/10000*15))
    ...
    table3 = doc.add_table(rows=16, cols=3)
    ...
    # Raices
    table3.cell(7,2).text=str( str(round((areas[6]*20000)/10000)) +' semillas')
    table3.cell(8,2).text=str('20 matas')
    table3.cell(9,2).text=str(str(round((areas[8]*2500)/10000)) +' matas')
    
    table3.cell(10,2).text=str(str(round((areas[9]*625)/10000)) +' matas')
    table3.cell(11,2).text=str(str(round((areas[10]*15)/10000)) +' libras')
    table3.cell(12,2).text=str(str(round((areas[11]*1110)/1000)) +' matas')
    table3.cell(13,2).text=str(str(round((areas[12]*15)/10000)) +' Libras')
    table3.cell(14,2).text=str(str(round((areas[13]*10)/10000)) +' Libras')
    
    table3.cell(15,0).text=str("Total area")
    table3.cell(15,1).text=str(sum(areas))
    ...
    #Dinero Tuberculos
    areasTuberculos=areas[6]+areas[7]+areas[8]+areas[9]
    TuberculosDinero=2*(areasTuberculos*150/10000)+areasTuberculos*120/10000
    # granos
    areasGranos=areas[10]+areas[11]+areas[12]+areas[13]
    GranosDinero=2*(areasGranos*120/10000)+areasGranos*105/10000

    # Total Beneficiarios
    TotalDinero=TuberculosDinero+GranosDinero+45

    #Tabla de jornaleros
    table5 = doc.add_table(rows=5, cols=2)
    table5.style = 'Table Grid'

    table5.cell(0,0).text=str('Rubro')
    table5.cell(0,1).text=str('Pago')
    table5.cell(1,0).text=str('Granos Básicos')
    table5.cell(2,0).text=str('Raíces y Tuberculos')
    table5.cell(3,0).text=str('Hortalizas')
    table5.cell(4,0).text=str('Total')
    table5.cell(1,1).text=str(round(GranosDinero,2))
    table5.cell(2,1).text=str(round(TuberculosDinero,2))
    table5.cell(3,1).text=str(45)
    table5.cell(4,1).text=str(round(TotalDinero,2))





    #dineroTotal=dineroTotal+45+round((areas[6]*10)/10000*15,2) + round((areas[7]*8)/10000*15,2) + round((areas[8]*10)/10000*15,2) + round((areas[9]*8)/10000*15,2) + round((areas[8]*10)/10000*15,2) + round((areas[8]*8)/10000*15,2) + round((areas[9]*7)/10000*15,2) + round((areas[9]*8)/10000*15,2) + round((areas[10]*7)/10000*15,2) + round((areas[10]*8)/10000*15,2) + round((areas[11]*7)/10000*15,2) + round((areas[11]*8)/10000*15,2) +round((areas[12]*7)/10000*15,2) + round((areas[12]*8)/10000*15,2) 
    
    #table3.cell(14,3).text=str(dineroTotal)

    name = name + round((areas[6]*20000)/10000)
    yuca = yuca + round((areas[8]*2500)/10000)

    platano = platano + round((areas[9]*625)/10000)
    arroz = arroz + round((areas[10]*15)/10000)
    guandu= guandu + round((areas[11]*1110)/10000)
    maiz= maiz + round((areas[12]*15)/10000)
    frijoles = frijoles + round((areas[13]*10)/10000)
    peonesTotal=peonesTotal+ TotalDinero
...
doc.save('Datos Beneficiarios final.docx')



print ('Resumen ')
print ( 'Pago total: '+ str(round(peonesTotal))) 
...
print ( 'semillas de nampi: '+ str(20*71))
